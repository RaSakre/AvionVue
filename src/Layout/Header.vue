<template>
  <Notify v-if="!isNotifyExist" />
  <header class="header">
    <div class="container">
      <div class="header__up-content">
        <div class="header__up-content-search">
          <button @click="toggleSearch" class="header__up-content-link" >
            <svg v-if="!isInputSearch" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="black">
              <path
          ...
          <Input v-if="isInputSearch" @blur="toggleSearch" @input="onSearch" :variant="'black'" :type="'text'"/>
        ...
        <div class="header__up-content-icons">
          <router-link to="/basket" id="basket" class="header__up-content-link">
            <span class="basket-counter">{{ basketLength }}</span>
            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14" viewBox="0 0 15 14" fill="black">
              ...
              <path
                ...
                fill="#2A254B" />
              <path
                d="M14 2.50001H2.91L2.5 0.40001C2.47662 0.285372 2.41379 0.182561 2.32243 0.109474C2.23107 0.0363867 2.11697 -0.0023488 2 1.01668e-05H0V1.00001H1.59L3.5 10.6C3.52338 10.7146 3.58621 10.8175 3.67757 10.8905C3.76893 10.9636 3.88303 11.0024 4 11H13V10H4.41L4 8.00001H13C13.1156 8.00284 13.2286 7.96552 13.3197 7.89441C13.4109 7.8233 13.4746 7.7228 13.5 7.61001L14.5 3.11001C14.5168 3.03582 14.5164 2.95879 14.4989 2.88478C14.4814 2.81076 14.4472 2.74172 14.399 2.6829C14.3508 2.62407 14.2898 2.57703 14.2206 2.54534C14.1515 2.51364 14.076 2.49814 14 2.50001ZM12.6 7.00001H3.81L3.11 3.50001H13.375L12.6 7.00001Z"
                fill="#2A254B" />
            </svg>
          </router-link>
          <a v-if="!authStore.isAuth" href="#" @click.prevent="toggleModal" class="header__up-content-link">
      ...
      <div class="header__down-content">
        <nav class="header__down-content-menu">
          <ul class="header__menu-list">
            ...
            <li class="header__menu-list-item">
              <a class="header__menu-list-link" href="#">Tables</a>
            </li>
            <li class="header__menu-list-item">
              <a class="header__menu-list-link" href="#">Chairs</a>
            </li>
            <li class="header__menu-list-item">
              <a class="header__menu-list-link" href="#">Crockery</a>
            </li>
            <li class="header__menu-list-item">
              <a class="header__menu-list-link" href="#">Tableware</a>
            </li>
            <li class="header__menu-list-item">
              <a class="header__menu-list-link" href="#">Cutlery</a>
            </li>
          </ul>
        </nav>
      </div>
...
<script setup>
  ...
  const isNotifyExist = ref(true);
  const isInputSearch = ref(false)
  const isModalOpen = ref(false);

  const toggleSearch = () => {
    isInputSearch.value = !isInputSearch.value;
  }
...
onMounted(() => {
  const isNotifyClosed = localStorage.getItem('notify') === 'closed';
  isNotifyExist.value = isNotifyClosed;
});
</script>
<style scoped>
  ...
  @media (max-width: 767px) {
    ...
    .header__up-content::after {
      display: none;
    }
    .header__up-content-icons {
      display: none;
    }
  }
  .header {
  ...
  @media (max-width: 767px) {
    .header__down-content-menu {
      ...
      animation: burgerAnimation 0.3s ease-in-out;
    }

    .header__down-content-menu .header__menu-list {
      display: flex;
      flex-direction: column;
